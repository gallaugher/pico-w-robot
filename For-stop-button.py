# Use this codee if you use a stop button (good for working with mobile).
# Also, add a blank space in Dashboard block for the "Release" button to avoid sending message upon release.
# This is important for iOS mobile since the button press doesn't send until the finger actually
# comes up off the button - simultaneously sending a press & release. So it's best to ignore release.
# This will also lower the time-til-throttle since you won't have extra "release" messages counting
# toward throttle cap.
...
path = "/sd/robot_sounds_named/" # name of folder on SD card containing audio files
...
def message(client, topic, message):
    ...
    if topic == sounds_feed: # sound button was pressed
        if message != "0": # ignore button release message "0"
            play_mp3(message)
    elif topic == move_feed: # robot directions
        if message == "stop":
            move_servo(0.0, 0.0)
